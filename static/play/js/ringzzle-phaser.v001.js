(function (root, factory) {
  ...
  class RingzzleScene extends SceneBase {
      ...
      for (let y = 0; y < BOARD_SIZE; y += 1) {
        for (let x = 0; x < BOARD_SIZE; x += 1) {
          ...
          const fill = isPreview
            ...
            : 0xf8fff9;
          const alpha = isPreview ? 0.22 : 0.10;
          g.fillStyle(fill, alpha);
          g.fillRoundedRect(cellX + pad, cellY + pad, cellSize - pad * 2, cellSize - pad * 2, radius);
          g.lineStyle(isFlash ? 3 : 1.5, isFlash ? 0xf2c94c : 0xffffff, isFlash ? 0.9 : 0.16);
    ...
    startDrag(slotIndex, pointer) {
      ...
      this.drawRing(ghost, 0, 0, piece.size, piece.color, this.layout.cellSize * 0.95, 0.86);
    ...
    handlePointerUp(pointer) {
      ...
      const clearLabel = result.clearEvents
        ? `${result.clearEvents > 1 ? "Combo clear" : "Clear"} +${result.scoreDelta}`
        : `Placed +${result.scoreDelta}`;
      this.statusText.setText(this.gameModel.gameOver ? "Game over." : clearLabel);
      this.render();
    ...
    renderGameOverPanel() {
      ...
      const title = this.add.text(centerX, centerY - 86, "Game Over", this.textStyle(25, "#13281f", "850", "center"));
      title.setOrigin(0.5, 0);
      const score = this.add.text(
        centerX,
        centerY - 42,
        `Score ${formatScore(this.gameModel.score)}`,
      ...
      const detail = this.add.text(
        centerX,
        centerY - 8,
        `Best ${formatScore(this.gameModel.highScore)}   Today ${formatScore(stats.todayBest)}   Games ${formatScore(stats.gamesPlayed)}`,
        this.textStyle(12, "#355245", "800", "center")
      );
      detail.setOrigin(0.5, 0);
      const restart = this.add.text(centerX, centerY + 48, "Restart", this.textStyle(15, "#f8fff9", "850", "center"))
        .setOrigin(0.5, 0)
        .setPadding(18, 10)
