(function (root, factory) {
      ...
      for (let y = 0; y < this.height; y += 1) {
        for (let x = 0; x < this.width; x += 1) {
          ...
          if (values.every((value) => value !== null) && values.every((value) => value === values[0])) {
            ...
            const event = { type: "color-burst", id: `cell-${x}-${y}`, color, x, y };
  ...
  class RingzzleScene extends SceneBase {
    ...
    playColorBurstCue(colorBurstEvents) {
      ...
      colorBurstEvents.forEach((event) => {
        const x = this.layout.boardOrigin.x + (event.x + 0.5) * this.layout.cellSize;
        const y = this.layout.boardOrigin.y + (event.y + 0.5) * this.layout.cellSize;
        const color = hexToNumber(COLORS[event.color % COLORS.length]);
        const pulse = this.add.circle(x, y, this.layout.cellSize * 0.42, color, 0.16);
        pulse.setStrokeStyle(Math.max(3, this.layout.cellSize * 0.035), color, 0.72);
        const text = this.add.text(x, y, "Color Burst", this.textStyle(15, "#f6fbff", "900", "center"));
        text.setOrigin(0.5, 0.5);
        text.setShadow(0, 2, "#000000", 5, false, true);
        this.feedbackLayer.add([pulse, text]);
        this.colorBurstItems.push(pulse, text);
      });
      if (typeof document !== "undefined") {
