(function (root, factory) {
  ...
  function calculateLayoutMetrics(width, height) {
    ...
    const statusPanelPad = Math.max(12, Math.floor(cellSize * 0.085));
    const statusChipY = tiny ? 82 : short ? 94 : compact ? 104 : 112;
    const statusChipHeight = tiny ? 26 : compact ? 30 : 36;
    const statusChipBottom = statusChipY + statusChipHeight / 2;
    const boardPanelTop = boardY - statusPanelPad;
    const statusY = Math.floor(clamp(
      (statusChipBottom + boardPanelTop - statusHeight) / 2,
      statusChipBottom,
      boardPanelTop - statusHeight
    ));
