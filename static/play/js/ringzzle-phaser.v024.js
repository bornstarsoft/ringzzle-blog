(function (root, factory) {
  ...
  function resolveLeaderboardViewState(previousState, action) {
    ...
    if (type === "loading") {
      const scope = action.scope === "alltime" ? "alltime" : "today";
      return { ...state, scope, loadingScope: scope, status: "Loading leaderboard...", needsLoad: false };
    }
    if (type === "loaded") {
      const scope = action.scope === "alltime" ? "alltime" : "today";
      const entries = { ...state.entries, [scope]: Array.isArray(action.entries) ? action.entries : [] };
      return { ...state, scope, entries, loadingScope: null, status: entries[scope].length ? "" : "No submitted scores yet.", needsLoad: false };
    }
    if (type === "error") {
      const scope = action.scope === "alltime" ? "alltime" : "today";
      const entries = { ...state.entries, [scope]: [] };
      return { ...state, scope, entries, loadingScope: null, status: action.message || "Leaderboard is temporarily unavailable.", needsLoad: false };
    }
    if (type === "submit-success") {
      return { ...state, entries: { today: null, alltime: null }, needsLoad: true };
    }
    return state;
  }
  ...
  class RingzzleScene extends SceneBase {
    ...
    buildLeaderboardSubmitDom() {
      if (typeof document === "undefined" || this.domSubmitPanel) return;
      const panel = document.createElement("section");
      panel.id = "ringzzle-leaderboard-submit";
      panel.className = "ringzzle-submit-panel";
      panel.hidden = true;
      panel.setAttribute("aria-live", "polite");
      panel.innerHTML = [
        '<label class="ringzzle-submit-label" for="ringzzle-leaderboard-nickname">Nickname</label>',
        '<div class="ringzzle-submit-row">',
        '<input id="ringzzle-leaderboard-nickname" class="ringzzle-submit-input" type="text" inputmode="text" autocomplete="nickname" maxlength="16" placeholder="Nickname" />',
        '<button id="ringzzle-leaderboard-submit-button" class="ringzzle-submit-button" type="button">Submit Score</button>',
        "</div>",
        '<p class="ringzzle-submit-privacy">Anonymous leaderboard. No login or email.</p>',
        '<p id="ringzzle-leaderboard-submit-message" class="ringzzle-submit-message"></p>',
      ].join("");
      document.body.appendChild(panel);
      this.domSubmitPanel = panel;
      this.domNicknameInput = panel.querySelector("#ringzzle-leaderboard-nickname");
      this.domSubmitButton = panel.querySelector("#ringzzle-leaderboard-submit-button");
      this.domSubmitMessage = panel.querySelector("#ringzzle-leaderboard-submit-message");
      if (this.domNicknameInput) {
        try {
          this.domNicknameInput.value = window.localStorage.getItem(STORAGE_KEYS.leaderboardNickname) || "";
        } catch (error) {
          // Local nickname preference is optional.
        }
      }
      this.domSubmitHandler = () => this.submitLeaderboardScore();
      if (this.domSubmitButton) this.domSubmitButton.addEventListener("click", this.domSubmitHandler);
      this.domNicknameFocusHandler = () => {
        if (this.domSubmitPanel) this.domSubmitPanel.classList.add("is-keyboard");
        window.setTimeout(() => this.syncLeaderboardSubmitPanel(), 0);
        window.setTimeout(() => this.syncLeaderboardSubmitPanel(), 120);
      };
      this.domNicknameBlurHandler = () => {
        if (this.domSubmitPanel) this.domSubmitPanel.classList.remove("is-keyboard");
        window.setTimeout(() => this.syncLeaderboardSubmitPanel(), 60);
        window.setTimeout(() => this.syncLeaderboardSubmitPanel(), 260);
      };
      if (this.domNicknameInput) {
        this.domNicknameInput.addEventListener("focus", this.domNicknameFocusHandler);
        this.domNicknameInput.addEventListener("blur", this.domNicknameBlurHandler);
      }
      document.documentElement.dataset.ringzzleLeaderboardSubmitDomReady = "true";
    }
    ...
    async loadLeaderboard(scope) {
      const resolvedScope = scope === "alltime" ? "alltime" : "today";
      if (this.leaderboardViewState && this.leaderboardViewState.loadingScope === resolvedScope) return;
      ...
      this.leaderboardViewState = resolveLeaderboardViewState(this.leaderboardViewState, { type: "loading", scope: resolvedScope });
      try {
    ...
    syncLeaderboardSubmitPanel() {
      if (!this.domSubmitPanel || !this.layout || !this.gameModel) return;
      const visible = !!(this.gameModel.gameOver && this.leaderboardSubmitState && this.leaderboardSubmitState.visible);
      this.domSubmitPanel.hidden = !visible;
      this.domSubmitPanel.dataset.visible = String(visible);
      if (!visible) {
        if (typeof document !== "undefined") {
          document.documentElement.dataset.ringzzleLeaderboardSubmitVisible = "false";
          document.documentElement.dataset.ringzzleLeaderboardSubmitDisabled = "true";
        }
        return;
      }

      const visualViewportHeight = typeof window !== "undefined" && window.visualViewport
        ? Math.floor(window.visualViewport.height || this.layout.height)
        : this.layout.height;
      const keyboardVisible = !!(this.domNicknameInput && typeof document !== "undefined" && document.activeElement === this.domNicknameInput)
        && visualViewportHeight < this.layout.height - 80;
      const submitLayout = calculateGameOverSubmitPanelLayout({
        layout: this.layout,
        keyboardVisible,
        visualViewportHeight,
      });
      this.domSubmitPanel.classList.toggle("is-keyboard", submitLayout.keyboardVisible);
      this.domSubmitPanel.style.left = `${submitLayout.left}px`;
      this.domSubmitPanel.style.top = `${submitLayout.top}px`;
      this.domSubmitPanel.style.width = `${submitLayout.width}px`;
      this.domSubmitPanel.style.maxHeight = `${submitLayout.maxHeight}px`;
      const submitting = !!(this.leaderboardSubmitState && this.leaderboardSubmitState.submitting);
      const canSubmit = !!(this.leaderboardSubmitState && this.leaderboardSubmitState.canSubmit);
      if (this.domSubmitButton) {
        this.domSubmitButton.disabled = !canSubmit || submitting;
        this.domSubmitButton.textContent = submitting ? "Submitting..." : this.leaderboardSubmitState.submitted ? "Submitted" : "Submit Score";
      }
      this.setLeaderboardSubmitMessage((this.leaderboardSubmitState && this.leaderboardSubmitState.message) || "");
      if (typeof document !== "undefined") {
        const doc = document.documentElement;
        doc.dataset.ringzzleLeaderboardSubmitVisible = "true";
        doc.dataset.ringzzleLeaderboardSubmitDisabled = String(!(canSubmit && !submitting));
        doc.dataset.ringzzleLeaderboardSubmitTop = String(submitLayout.top);
        doc.dataset.ringzzleLeaderboardSubmitLeft = String(submitLayout.left);
        doc.dataset.ringzzleLeaderboardSubmitBottom = String(submitLayout.bottom);
        doc.dataset.ringzzleLeaderboardSubmitMaxHeight = String(submitLayout.maxHeight);
        doc.dataset.ringzzleLeaderboardSubmitKeyboard = String(submitLayout.keyboardVisible);
        doc.dataset.ringzzleLeaderboardRestartTop = String(submitLayout.restartTop);
      }
    }

    async submitLeaderboardScore() {
      if (!this.gameModel || !this.gameModel.gameOver || !this.domNicknameInput) return;
      ...
      const nicknameResult = validateLeaderboardNicknameClient(this.domNicknameInput.value);
      ...
      try {
        ...
        try {
          window.localStorage.setItem(STORAGE_KEYS.leaderboardNickname, nicknameResult.nickname);
        } catch (error) {
    ...
    renderGameOverPanel() {
      ...
      const centerX = width / 2;
      const centerY = height / 2;
      const submitLayout = calculateGameOverSubmitPanelLayout({ layout: this.layout });
      const leaderboardLayout = calculateGameOverLeaderboardLayout({ layout: this.layout });
      const scope = this.leaderboardViewState.scope === "alltime" ? "alltime" : "today";
      const leaderboardTitle = scope === "today" ? "Today Top 100" : "All-Time Top 100";
      const leaderboardLines = this.getLeaderboardPreviewLines(leaderboardLayout.entryLimit);
      const panelTop = Math.max(18, centerY - (this.layout.compact ? 186 : 198));
      const panelBottom = Math.min(height - 10, submitLayout.restartTop + (this.layout.compact ? 48 : 52));
      const panelHeight = Math.max(this.layout.compact ? 410 : 430, panelBottom - panelTop);
      const scrim = this.add.rectangle(centerX, centerY, width + 8, height + 8, 0x03070d, 0.72).setInteractive();
      const panel = this.add.rectangle(centerX, panelTop + panelHeight / 2, panelWidth, panelHeight, 0x0b1622, 0.98);
      panel.setStrokeStyle(2, 0x35e6d0, 0.74);
      const title = this.add.text(centerX, centerY - (this.layout.compact ? 152 : 162), "Game Over", this.textStyle(25, "#f6fbff", "850", "center"));
      title.setOrigin(0.5, 0);
      const score = this.add.text(
        centerX,
        centerY - (this.layout.compact ? 114 : 122),
      ...
      const detail = this.add.text(
        centerX,
        centerY - (this.layout.compact ? 84 : 88),
      ...
      const leaderboardHeading = this.add.text(centerX, leaderboardLayout.headingY, `Anonymous leaderboard · ${leaderboardTitle}`, this.textStyle(12, "#35e6d0", "850", "center"));
      leaderboardHeading.setOrigin(0.5, 0);
      const todayButton = this.add.text(leaderboardLayout.todayButton.x, leaderboardLayout.todayButton.y, "Today", this.textStyle(11, scope === "today" ? "#07131e" : "#f6fbff", "850", "center"))
        .setOrigin(0.5, 0)
        .setPadding(10, 7)
        .setBackgroundColor(scope === "today" ? "#35e6d0" : "#102839")
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.setLeaderboardScope("today"));
      const allTimeButton = this.add.text(leaderboardLayout.allTimeButton.x, leaderboardLayout.allTimeButton.y, "All-Time", this.textStyle(11, scope === "alltime" ? "#07131e" : "#f6fbff", "850", "center"))
        .setOrigin(0.5, 0)
        .setPadding(10, 7)
        .setBackgroundColor(scope === "alltime" ? "#35e6d0" : "#102839")
        .setInteractive({ useHandCursor: true })
      ...
      const restart = this.add.text(centerX, submitLayout.restartTop, "Restart", this.textStyle(15, "#f6fbff", "850", "center"))
        .setOrigin(0.5, 0)
        .setPadding(18, 10)
        .setBackgroundColor("#173447")
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.restartGame());
      this.gameOverLayer.add([scrim, panel, title, score, detail, leaderboardHeading, todayButton, allTimeButton, leaderboardText, restart]);
      this.syncLeaderboardSubmitPanel();
    }
