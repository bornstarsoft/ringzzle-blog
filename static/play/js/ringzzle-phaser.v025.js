(function (root, factory) {
  ...
  class RingzzleScene extends SceneBase {
    ...
    renderGameOverPanel() {
      ...
      const nicknameResult = validateLeaderboardNicknameClient(storedNickname);
      const nicknameLine = nicknameResult.ok ? `Nickname: ${nicknameResult.nickname}` : "Tap Submit Score to choose nickname.";
      const submitMessage = this.leaderboardSubmitState && this.leaderboardSubmitState.message
        ? this.leaderboardSubmitState.message
        : "Anonymous leaderboard. No login or email.";
      ...
      const submitLabel = submitting ? "Submitting..." : submitted ? "Submitted" : "Submit Score";
      ...
      const nicknameText = this.add.text(panelLayout.nickname.x, panelLayout.nickname.y, nicknameLine, this.textStyle(10, "#bfd1dc", "800", "center"))
        .setOrigin(0.5, 0)
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.editLeaderboardNickname());
      nicknameText.setWordWrapWidth(panelLayout.panelWidth - 44);
      const submitButton = this.add.text(panelLayout.submitButton.x, panelLayout.submitButton.y, submitLabel, this.textStyle(13, submitted ? "#7ba0ad" : "#07131e", "850", "center"))
        .setOrigin(0.5, 0)
        .setPadding(20, panelLayout.compact ? 7 : 8)
        .setBackgroundColor(submitted ? "#173447" : "#35e6d0")
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.submitLeaderboardScore());
      submitButton.setAlpha(submitting || submitted ? 0.88 : 1);
