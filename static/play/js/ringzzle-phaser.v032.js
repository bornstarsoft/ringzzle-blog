(function (root, factory) {
  ...
  function normalizeAudioState(audioState) {
    return audioState || "";
  }
  ...
  function calculateHomeButtonLayout(options) {
    const opts = options || {};
    const width = Math.max(280, Math.floor(opts.width || 390));
    const margin = Math.max(10, Math.floor(opts.margin || 16));
    const topY = Math.floor(opts.topY || 12);
    const home = opts.homeButton || {};
    const sound = opts.soundButton || {};
    const buttonWidth = Math.max(42, Math.floor(home.width || 52));
    const buttonHeight = Math.max(26, Math.floor(home.height || 34));
    const gap = Math.max(5, Math.floor(opts.gap || 7));
    const soundX = Number.isFinite(sound.x)
      ? sound.x
      : width - margin - buttonWidth * 2 - gap;
    const soundY = Number.isFinite(sound.y) ? sound.y : topY + 2;
    const desiredX = soundX - buttonWidth - gap;
    return {
      label: "Home",
      route: HOME_ROUTE,
      action: "navigate-home",
      x: clamp(Math.round(desiredX), margin, Math.max(margin, width - margin - buttonWidth)),
      y: Math.round(soundY),
      width: buttonWidth,
      height: buttonHeight,
      gap,
  ...
  function getAudioUnlockStatus(soundState, audioState, confirmationScheduled) {
    const state = soundState || {};
    const normalizedAudioState = normalizeAudioState(audioState);
    const ready = Boolean(state.enabled && state.audioReady && normalizedAudioState === "running" && confirmationScheduled);
    return {
      ready,
      pending: Boolean(state.userActivated && !ready && !state.blocked),
      blocked: Boolean(state.blocked || (state.enabled && normalizedAudioState && normalizedAudioState !== "running")),
    };
  }

  function getSoundPlaybackIntent(soundState, audioState, eventName, options) {
    const opts = options || {};
    const cue = getSoundCueSpec(eventName);
    const normalizedAudioState = normalizeAudioState(audioState);
    const force = !!opts.force;
    const hasUsableContext = !!normalizedAudioState && normalizedAudioState !== "closed";
    const enabled = !!(soundState && soundState.enabled);
    return {
      recognized: !!cue,
      shouldAttempt: !!cue && hasUsableContext && (force || (enabled && normalizedAudioState === "running")),
      blocked: !!cue && enabled && normalizedAudioState !== "running",
      force,
    };
  }
  ...
  function getPlayEmbedMode(locationLike) {
  ...
  function getEmbedUiOptions(embedMode) {
    const isHome = embedMode === EMBED_MODE_HOME;
    return {
      showTitle: !isHome,
      showSubtitle: !isHome,
      showHomeButton: !isHome,
      showRankButton: !isHome,
  ...
  function calculateLayoutMetrics(width, height) {
    ...
    const trayGap = realSafariCompact ? (tiny ? 34 : 38) : tiny ? 24 : compact ? 28 : 30;
  ...
  function getSoundButtonDisplayWidth(button) {
    if (!button) return SOUND_BUTTON_MIN_WIDTH;
    return Math.max(SOUND_BUTTON_MIN_WIDTH, Math.ceil(button.width || 0));
  }

  function getControlButtonDisplayHeight(button) {
    if (!button) return CONTROL_BUTTON_MIN_HEIGHT;
    return Math.max(CONTROL_BUTTON_MIN_HEIGHT, Math.ceil((button.height || 0) + 6));
  }
  ...
  class RingzzleScene extends SceneBase {
    constructor() {
      ...
      this.domSoundButtonEventName = null;
      ...
      this.embedMode = getPlayEmbedMode(root.location);
      this.embedUiOptions = getEmbedUiOptions(this.embedMode);
    }
    ...
    buildStaticUi() {
      const embedUiOptions = this.embedUiOptions || getEmbedUiOptions(this.embedMode);
      this.titleText = this.add.text(0, 0, embedUiOptions.showTitle ? "Ringzzle" : "", this.textStyle(34, "#f6fbff", "900"));
      this.titleText.setShadow(0, 2, "#000000", 4, false, true);
      this.subtitleText = this.add.text(
        0,
        0,
        embedUiOptions.showSubtitle ? "Free color rings puzzle game. No install. Just play." : "",
      ...
      this.homeButton = this.add.text(0, 0, embedUiOptions.showHomeButton ? "Home" : "", this.textStyle(12, "#f6fbff", "850"))
        .setPadding(9, 6)
        .setBackgroundColor("#102839")
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.goHome());
      this.rankButton = this.add.text(0, 0, embedUiOptions.showRankButton ? "Rank" : "", this.textStyle(12, "#f6fbff", "850"))
        .setPadding(9, 6)
        .setBackgroundColor("#102839")
        .setInteractive({ useHandCursor: true })
      ...
      if (typeof document !== "undefined") {
        ...
        if (this.domSoundButton) {
          this.domSoundButtonEventName = typeof window !== "undefined" && "PointerEvent" in window ? "pointerdown" : "click";
          this.domSoundButtonHandler = (event) => {
            if (event && typeof event.preventDefault === "function" && event.cancelable) event.preventDefault();
            this.toggleSound();
          };
          this.domSoundButton.addEventListener(this.domSoundButtonEventName, this.domSoundButtonHandler);
          document.documentElement.dataset.ringzzleSoundDomButtonReady = "true";
          document.documentElement.dataset.ringzzleSoundDomButtonEvent = this.domSoundButtonEventName;
        }
      ...
      this.applyEmbedUiMode();
    }

    applyEmbedUiMode() {
      const options = this.embedUiOptions || getEmbedUiOptions(this.embedMode);
      this.titleText.setVisible(!!options.showTitle);
      this.subtitleText.setVisible(!!options.showSubtitle);
      this.homeButton.setVisible(!!options.showHomeButton);
      this.rankButton.setVisible(!!options.showRankButton);
      this.restartButton.setVisible(!!options.showRestartButton);
      this.soundButton.setVisible(!!options.showSoundButton);
      if (options.showHomeButton) {
        this.homeButton.setInteractive({ useHandCursor: true });
      } else {
        this.homeButton.disableInteractive();
      }
      if (options.showRankButton) {
        this.rankButton.setInteractive({ useHandCursor: true });
      } else {
        this.rankButton.disableInteractive();
      }
    }
    ...
    removeViewportListeners() {
      ...
      if (this.domSoundButton && this.domSoundButtonHandler) {
        this.domSoundButton.removeEventListener(this.domSoundButtonEventName || "click", this.domSoundButtonHandler);
      }
    ...
    resizeLayout() {
      ...
      this.titleText.setText(embedUiOptions.showTitle ? "Ringzzle" : "");
      this.subtitleText.setText(embedUiOptions.showSubtitle ? (compact ? "Free color rings puzzle game." : "Free color rings puzzle game. No install. Just play.") : "");
      this.homeButton.setText(embedUiOptions.showHomeButton ? "Home" : "");
      this.rankButton.setText(embedUiOptions.showRankButton ? "Rank" : "");
      ...
      this.homeButton.setFontSize(tiny ? 11 : compact ? 12 : 13);
      this.homeButton.setPadding(compact ? 8 : 10, compact ? 6 : 7);
      ...
      const soundButtonWidth = getSoundButtonDisplayWidth(this.soundButton);
      const soundX = Math.max(margin, restartX - soundButtonWidth - buttonGap);
      this.restartButton.setPosition(restartX, topY + 2);
      this.soundButton.setPosition(soundX, topY + 2);
      const homeLayout = calculateHomeButtonLayout({
        width,
        margin,
        topY,
        homeButton: {
          width: this.homeButton.width,
          height: this.homeButton.height,
        },
        soundButton: {
          x: this.soundButton.x,
          y: this.soundButton.y,
          width: soundButtonWidth,
          height: getControlButtonDisplayHeight(this.soundButton),
        },
        gap: buttonGap,
      });
      this.homeButton.setPosition(homeLayout.x, homeLayout.y);
      ...
      const subtitleButtonReserve = this.restartButton.width
        + this.soundButton.width
        + (embedUiOptions.showHomeButton ? this.homeButton.width + buttonGap : 0)
        + buttonGap
        + 18;
      this.subtitleText.setWordWrapWidth(Math.max(160, width - margin * 2 - subtitleButtonReserve));
    ...
    publishLayoutMetrics() {
      ...
      doc.dataset.ringzzleEmbedMode = this.embedMode || "";
      doc.dataset.ringzzleTitleVisible = String(!!(this.titleText && this.titleText.visible));
      doc.dataset.ringzzleSubtitleVisible = String(!!(this.subtitleText && this.subtitleText.visible));
      doc.dataset.ringzzleHomeVisible = String(!!(this.homeButton && this.homeButton.visible));
      doc.dataset.ringzzleRankVisible = String(!!(this.rankButton && this.rankButton.visible));
      doc.dataset.ringzzleRestartVisible = String(!!(this.restartButton && this.restartButton.visible));
      doc.dataset.ringzzleTitleText = this.titleText ? this.titleText.text : "";
      doc.dataset.ringzzleSubtitleText = this.subtitleText ? this.subtitleText.text : "";
      doc.dataset.ringzzleHomeText = this.homeButton ? this.homeButton.text : "";
      doc.dataset.ringzzleRankText = this.rankButton ? this.rankButton.text : "";
    ...
    publishSoundMetrics() {
      ...
      doc.dataset.ringzzleSoundEnabled = String(!!(this.soundState && this.soundState.enabled));
      doc.dataset.ringzzleSoundUserActivated = String(!!(this.soundState && this.soundState.userActivated));
      doc.dataset.ringzzleSoundAudioReady = String(!!(this.soundState && this.soundState.audioReady));
      doc.dataset.ringzzleSoundBlocked = String(!!(this.soundState && this.soundState.blocked));
      doc.dataset.ringzzleAudioContextState = this.audioContext ? this.audioContext.state : "";
      doc.dataset.ringzzleSoundLabel = getSoundButtonLabel(this.soundState);
      if (this.soundButton) {
        const soundButtonWidth = getSoundButtonDisplayWidth(this.soundButton);
        const soundButtonHeight = getControlButtonDisplayHeight(this.soundButton);
        ...
        doc.dataset.ringzzleSoundButtonWidth = String(Math.round(soundButtonWidth));
        doc.dataset.ringzzleSoundButtonHeight = String(Math.round(soundButtonHeight));
      }
      if (this.homeButton) {
        doc.dataset.ringzzleHomeButtonX = String(Math.round(this.homeButton.x));
        doc.dataset.ringzzleHomeButtonY = String(Math.round(this.homeButton.y));
        doc.dataset.ringzzleHomeButtonWidth = String(Math.round(this.homeButton.width));
        doc.dataset.ringzzleHomeButtonHeight = String(Math.round(this.homeButton.height));
        doc.dataset.ringzzleHomeRoute = HOME_ROUTE;
      }
    }
    ...
    syncDomSoundButton() {
      ...
      this.domSoundButton.style.width = `${Math.round(getSoundButtonDisplayWidth(this.soundButton))}px`;
      this.domSoundButton.style.height = `${Math.round(getControlButtonDisplayHeight(this.soundButton))}px`;
    ...
    async activateAudioContextFromGesture() {
      ...
      try {
        ...
        const scheduledBeforeResume = this.playSoundCue("toggle-on", { force: true });
        if (this.audioContext.state === "suspended" && typeof this.audioContext.resume === "function") {
          await this.audioContext.resume();
        }
        const scheduledAfterResume = scheduledBeforeResume || this.playSoundCue("toggle-on", { force: true });
        return this.audioContext.state === "running" && scheduledAfterResume;
      } catch (error) {
        this.audioContext = null;
        return false;
      }
    ...
    async toggleSound() {
      ...
      const audioReady = await this.activateAudioContextFromGesture();
    ...
    playSoundCue(eventName, options) {
      ...
      const audioState = this.audioContext ? this.audioContext.state : "";
      const intent = getSoundPlaybackIntent(this.soundState, audioState, eventName, opts);
      if (!spec || !this.audioContext || !intent.shouldAttempt) {
        if (intent.blocked && this.soundState) {
          this.soundState = {
            ...this.soundState,
            enabled: false,
            audioReady: false,
  ...
  const RingzzleCore = {
    ...
    getSoundButtonDisplayWidth,
    getControlButtonDisplayHeight,
