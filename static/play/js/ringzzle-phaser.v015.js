(function (root, factory) {
  ...
  function calculateTrayRingRenderMetrics(layout) {
    ...
    const ringCellSize = Math.floor(wellSize * TRAY_RING_RENDER_SCALE);
    const largestRingDiameter = ringCellSize * getRingRadiusRatio("large") * 2;
    return {
      wellSize,
      ringCellSize,
      largestRingDiameter,
      margin: (wellSize - largestRingDiameter) / 2,
      ringScale: TRAY_RING_RENDER_SCALE,
    };
  }
  ...
  class RingzzleScene extends SceneBase {
    ...
    renderTray() {
      ...
      for (let index = 0; index < TRAY_SIZE; index += 1) {
        ...
        if (piece && !isDragging) {
          const metrics = calculateTrayRingRenderMetrics(layout);
          this.drawRing(g, center.x, center.y - 2, piece.size, piece.color, metrics.ringCellSize, this.gameModel.gameOver ? 0.34 : 1);
        }
